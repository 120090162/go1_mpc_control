...
int main(int, char **)
{
    ...
    std::cout << "rotation matrix from quat" << std::endl
              << quat.toRotationMatrix() << std::endl;

    Eigen::Quaterniond quat2 = Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(pitch, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(roll, Eigen::Vector3d::UnitX());
    std::cout << "rotation matrix from r p y  is " << std::endl
              << quat2.toRotationMatrix() << std::endl;
    // quat2 should equals to quat

    Eigen::Quaterniond quat3 = Eigen::AngleAxisd(yaw2, Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(pitch2, Eigen::Vector3d::UnitY()) * Eigen::AngleAxisd(roll2, Eigen::Vector3d::UnitX());
    std::cout << "rotation matrix from r2 p2 y2  is " << std::endl
              << quat3.toRotationMatrix() << std::endl;
    ...
    std::cout << "ang_vel_to_rpy_rate  " << std::endl
              << ang_vel_to_rpy_rate << std::endl;
